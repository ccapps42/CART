...
The $d{=}1024$ result (Section~\ref{sec:hyp_vs_outcome}) raises a focused question: which of \cart{}'s architectural choices is responsible for the gap with Dense? We treat the three most directly testable hypotheses --- frozen $K, V$ from the prelude, useful contribution from loop iterations beyond the first, and shared identical weights across all $R$ iterations --- as candidate bottlenecks and report single-seed ablations against each. Each ablation trains a variant at $d{=}1024, R{=}6, P{=}6, \text{seed}{=}42$ (the diagnostic corner where the Dense gap is cleanest) with all other hyperparameters identical to the baseline. Statistical caveats apply: with one seed per ablation versus three for the baseline, only effects exceeding the baseline's seed-noise band ($\sim 0.08$ on \pplwiki) are interpretable as signal.
...
Second, the unshared variant still loses to Dense 12L at parameter parity. The architectural distinctness that survives the removal of weight sharing --- cross-attention against a fixed prelude-derived $K, V$ anchor, in place of self-attention with per-layer $K, V$ recomputation, embedded in a heterogeneous prelude $\to$ anchor $\to$ core $\to$ coda structure --- caps the architecture below a vanilla self-attention transformer of equivalent depth. The $d{=}1024$ gap to Dense thus has two distinct architectural contributors: roughly $5\%$ from shared weights (now diagnosed by this ablation), and roughly $5\%$ from the prelude+anchor+core+coda template itself. Neither alone explains the full deficit.
...
\begin{enumerate}[leftmargin=*, topsep=2pt, itemsep=1pt]
  ...
  \item The $d{=}1024$ \cart{}--Dense gap decomposes across three architectural mechanisms via diagnostic ablations (Section~\ref{sec:ablations}). Frozen $K, V$ is exonerated: recomputing $K, V$ from $h$ at every iteration ties the baseline within the seed-noise band (Section~\ref{sec:abl_kv}). Recurrence beyond the first iteration contributes $\sim 1\%$: at $R{=}1$, \pplwiki rises by $1.1\%$ versus $R{=}6$, indicating the looped core captures only marginal additional capacity (Section~\ref{sec:abl_r1}). Shared weights cost $\sim 5\%$: unrolling the core into $R$ unique blocks improves \pplwiki by $5.6\%$ over baseline (Section~\ref{sec:abl_unshared}). The remaining $\sim 5\%$ gap to Dense 12L at parameter parity comes from the cross-attention-against-prelude-anchor template that survives even the unshared variant. \cart{}'s recurrent-with-shared-weights formulation does not deliver the parameter-efficiency it was designed to deliver at this scale and training budget.
\end{enumerate}

\paragraph{What remains unresolved.}
The ablations attribute most of the $d{=}1024$ gap to two factors --- shared weights ($\sim 5\%$) and the cross-attention-against-anchor template ($\sim 5\%$). The remaining architectural question is whether the cross-attention/anchor structure \emph{specifically} caps the architecture, or whether some other component of the heterogeneous prelude $\to$ anchor $\to$ core $\to$ coda framing is the cap. A targeted ablation replacing cross-attention with self-attention (and adding per-layer $K, V$ recomputation) in the unshared core would isolate this. \tbd{This is left to future work.} The combined picture supports a tempered conclusion: \cart{}'s recurrent-with-shared-weights formulation does not deliver the parameter-efficiency it was designed to deliver at $d{=}1024$ on $\sim 1$B tokens, but the architecture's other contributions --- deterministic latency, fixed VRAM, the spectral-radius emergence finding (Section~\ref{sec:spectral}), the Stage 1 $\to$ Stage 2 $R$-benefit reversal --- survive intact.
