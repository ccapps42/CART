...
\begin{enumerate}[leftmargin=*, topsep=2pt, itemsep=1pt]
  \item \textbf{Universal learned memory timescale.} Across all 64 Stage 1 configurations (four model widths $d \in \{256, 512, 768, 1024\}$ with $R \in \{2, 4, 6, 8\}$ and $P \in \{2, 3, 4, 6\}$), the LTI gate's spectral radius converges to $\varrho \approx 0.893$, indistinguishable from the $R{=}6$ ``ideal'' value $0.5^{1/R} = 0.8909$. The convergence rate adapts gradually with model width ($+0.0042$ from $d{=}256$ to $d{=}1024$) but is essentially insensitive to loop count.
...
\paragraph{Token budget and Chinchilla compliance.} Stage 2 trains every configuration on $\sim 1$B tokens. The corresponding tokens-per-effective-parameter ratio varies across the sweep:

\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=1pt]
  \item $d{=}256$: $47$--$56:1$ — over-Chinchilla (smaller models trained well past the $20:1$ optimum).
  \item $d{=}512$: $13$--$18:1$ — approximately Chinchilla-optimal at this scale.
  \item $d{=}768$: $7$--$10:1$ — sub-Chinchilla.
  \item $d{=}1024$: $4.5$--$5.6:1$ — well under Chinchilla, at roughly $25\%$ of the optimal token budget.
\end{itemize}

This means cross-scale comparisons in this paper are not measured at matched-Chinchilla budgets. The $d{=}1024$ models in particular are likely to improve substantially with more tokens; their reported perplexities are lower bounds on quality, not asymptotic values. For context, recent sub-1B language models trained for downstream deployment are typically far over-Chinchilla: TinyLlama-1.1B~\citep{zhang2024tinyllama} trains on 3T tokens ($\sim 2700{:}1$), Pythia-160M~\citep{biderman2023pythia} trains on 300B tokens ($\sim 1875{:}1$). Our $4.5{-}56{:}1$ range reflects the goals of an architectural sweep rather than a deployment-targeted release. We discuss this further in Limitations (Section~\ref{sec:discussion}).
...
At the full Stage 2 training budget of $\sim 1$B tokens, the partial data already complete ($d{=}256$ and $d{=}512$, mean across $R \in \{6, 8, 10\}$ and 3 seeds each) show a steeper exponent:
\[
\frac{\text{PPL}_\text{wiki}^\text{stage2}(512)}{\text{PPL}_\text{wiki}^\text{stage2}(256)} \;\approx\; \frac{27.2}{40.7} \;\approx\; 0.667
\]
implying a single-doubling exponent of approximately $-0.58$. If this exponent holds at $d \in \{768, 1024\}$, it would suggest \cart{}'s per-$d$ efficiency \emph{improves} with training budget (width pays off more under longer training), rather than collapsing to a single budget-invariant exponent.

\begin{table}[htbp]
\centering
\caption{Cross-scale \pplwiki{} (mean across seeds and $R$ at $P=6$) and implied scaling. Stage 1 is the four-scale fit; Stage 2 is single-doubling exponent computed from adjacent scales.}
\label{tab:scaling}
\footnotesize
\begin{tabular}{rrrr}
\toprule
$d$ & Stage 1 \pplwiki & Stage 2 \pplwiki & Implied exp.\ ($d$ to $2d$) \\
\midrule
 256 & 186.20 & 40.7   & ---                  \\
 512 & 136.42 & 27.2   & $-0.58$              \\
 768 & 114.96 & \tbd{} & \tbd{}               \\
1024 &  97.73 & \tbd{} & \tbd{}               \\
\bottomrule
\end{tabular}
...
We evaluate this property by training a single Stage 2 model at $R{=}10$ and evaluating perplexity at $R \in \{4, 6, 8, 10, 12, 14, 16\}$ on all three held-out sets, with no fine-tuning. Results are \tbd{pending completion of $d{=}1024$ Stage 2 runs}, including: (i) the marginal perplexity gain from each additional inference-time loop; (ii) the saturation point at which additional loops cease to improve perplexity; and (iii) a comparison against models trained at the same target $R$, isolating the cost of train/inference $R$ mismatch. If the saturation point is well above the trained $R$, this provides a deterministic-latency analogue to the variable-compute scaling reported by~\citet{geiping2025latent}.

\subsection{Downstream Benchmarks}
\label{sec:benchmarks}

Perplexity establishes that \cart{} produces a calibrated language model, but downstream task accuracy is the harder test. We evaluate the complete Stage 2 sweep on four standard zero-shot benchmarks via the LM Evaluation Harness~\citep{eval-harness}: HellaSwag (commonsense completion), ARC-Challenge (grade-school science), LAMBADA (long-range coreference), and PIQA (physical commonsense). Results are \tbd{pending Stage 2 completion} and will be reported alongside the Dense baseline at parameter parity.
...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=1pt]
  ...
  \item \textbf{Sub-Chinchilla token budget at large scales.} Stage 2 trains every configuration on $\sim 1$B tokens. The tokens-per-effective-parameter ratio is over-Chinchilla at $d{=}256$ ($47$--$56:1$), approximately Chinchilla-optimal at $d{=}512$ ($13$--$18:1$), and well under Chinchilla at $d{=}1024$ ($4.5$--$5.6:1$, see Section~\ref{sec:repro}). Cross-scale comparisons in this paper are therefore not measured at matched-token-budget conditions; large-$d$ \cart{} numbers are likely lower bounds on asymptotic quality.
