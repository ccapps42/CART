...
To address this, a planned follow-up experiment resumes training from the best $d{=}1024$ $R{=}10$ checkpoint and continues for an additional 45{,}750 steps to a total of 2.5B tokens (exactly $20{:}1$ on stored parameters; $9$--$11{:}1$ on effective parameters depending on $R$). The additional training data is a fresh 1.5B-token chunk to be built from corpus shards not used in the Stage 2 build, preserving the single-pass, no-repetition property. We plan to resume training (rather than train from scratch) to isolate the effect of additional tokens on a known checkpoint and to keep total compute manageable ($\sim 14$ hours additional on a single RTX 3090).

The planned extended-training run targets three specific questions:
