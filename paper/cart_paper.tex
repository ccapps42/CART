...
At the full Stage 2 training budget of $\sim 1$B tokens, the cross-scale picture is now visible at all four scales, with $R{=}6$ means available everywhere (one seed at $d{=}1024$, three seeds at $d \leq 768$). Reporting $R{=}6$ rather than R-averaged means keeps the cross-scale comparison clean, since $R{=}6$ was the best $R$ at every scale tested so far.

The Stage 2 single-step exponents on \pplwiki{} soften progressively with scale: $-0.60$ from $d{=}256 \to d{=}512$, $-0.42$ from $d{=}512 \to d{=}768$, and $-0.39$ from $d{=}768 \to d{=}1024$ (first seed). A 4-point power-law fit on the available R=6 means yields $\text{PPL}_\text{wiki}^\text{stage2}(d) \approx 668 \cdot d^{-0.51}$, somewhat steeper than the Stage 1 fit of $-0.458$ but with visible softening at the larger end. \tbd{The d=1024 entry is a single seed; the 3-seed mean and the R=8/R=10 means at d=1024 will update this section once Stage 2 completes (est.\ 2026-05-16).}
...
\begin{table}[htbp]
\centering
\caption{Cross-scale \pplwiki{} at $R{=}6$, $P{=}6$ across Stage 1 (step 3{,}000) and Stage 2 (step 30{,}500). Stage 2 means: $d \leq 768$ are 3-seed means; $d{=}1024$ is a single seed (pending 2 more). Implied exponent computed from the prior scale entry.}
\label{tab:scaling}
\footnotesize
\begin{tabular}{rrrl}
\toprule
...
1024 &  97.73 & 20.45 (1 seed, \tbd{})    & $-0.39$ ($\times 1.33$) \\
\bottomrule
\end{tabular}
\end{table}
