...
At the full Stage 2 training budget of $\sim 1$B tokens, the cross-scale picture is now available at all four scales as 3-seed $R{=}6$ means. Reporting $R{=}6$ rather than R-averaged means keeps the cross-scale comparison clean, since $R{=}6$ was the best $R$ at every scale tested so far.

The Stage 2 single-step exponents on \pplwiki{} soften progressively with scale: $-0.60$ from $d{=}256 \to d{=}512$, $-0.42$ from $d{=}512 \to d{=}768$, and $-0.40$ from $d{=}768 \to d{=}1024$. A 4-point power-law fit on the R=6 means yields $\text{PPL}_\text{wiki}^\text{stage2}(d) \approx 671 \cdot d^{-0.51}$, steeper than the Stage 1 fit of $-0.46$ but with visible softening at the larger end. 3-seed agreement at d=1024 R=6 is tight (std on ppl\_wiki across seeds: $0.07$, on ppl\_tiny: $0.010$). \tbd{The R=8 and R=10 sweeps at d=1024 will update the cross-R picture and may shift the per-scale best $R$ at d=1024; final scaling fit pending Stage 2 completion (est.\ 2026-05-16).}

Two interpretations of the softening are consistent with the current data: (a) the cross-scale fit is approximately $-0.50$ with some variance at each transition, and the softening is sample noise; or (b) per-$d$ efficiency genuinely diminishes above $d{=}512$, signaling early saturation of the prelude representation under the Stage 2 token budget. Interpretation (b) is becoming the more parsimonious read as each successive scale step delivers a shallower exponent, but distinguishing the two requires either the d=1024 3-seed mean or the extended-training experiment (Section~\ref{sec:discussion}). \tbd{Final interpretation pending d=1024 full sweep and the extended-training run.}
